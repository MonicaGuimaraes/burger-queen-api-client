...
export default function Command({order, onClickStatus, classNameButton, children, disabled}) {
  ...
  return(
    <li key={order.id} className={styles.sectionCommand}>
      ...
      <h3>{order.status === 'ready' || order.status === 'delivered' ? comparingTheTime(order.createdAt, order.updatedAt) : ""}</h3>
      <h3>Mesa: {order.table}</h3>
      {order.Products.map((product, index) => {
        const name = standardizeName(product)
        return ( <p key={index}>{product.qtd}X {name}</p> )
      })}
      <button className={classNameButton} disabled={disabled} onClick={onClickStatus}>{children}</button>
    </li>
  )
}